...
class CStruct : REPR() {
    ...
    override fun compose(tc: ThreadContext, st: STable, repr_info: SixModelObject) {
        ...
        val repr_data = CStructREPRData()

        val mroLength = attr_info.elems(tc)
        val attrInfos = ArrayList<CStructREPRData.AttrInfo>()
        for (i in mroLength - 1 downTo 0) {
            ...
            if (parents <= 1) {
                ...
                for (j in 0 until numAttrs) {
                    ...
                    val info = CStructREPRData.AttrInfo()
    ...
    override fun allocate(tc: ThreadContext, st: STable): SixModelObject {
         ...
         * composed. */
        val obj = CStructInstance()
        val repr_data = st.REPRData as CStructREPRData
        obj.st = st
        try {
    ...
    @Suppress("DEPRECATION")
    private fun generateStructClass(tc: ThreadContext, st: STable, fields: List<CStructREPRData.AttrInfo>) {
        val reprData = st.REPRData as CStructREPRData
        val cw = ClassWriter(ClassWriter.COMPUTE_MAXS or ClassWriter.COMPUTE_FRAMES)
        val className = "__CStruct__" + typeId++
    ...
    private fun typeDescriptor(tc: ThreadContext, info: CStructREPRData.AttrInfo): String? {
