 ...
 */
class VMArrayInstance : VMArrayInstanceBase() {
    companion object {
    ...
    override fun at_pos_boxed(tc: ThreadContext, index: Long): SixModelObject? {
        ...
        return slots!![start + index.toInt()]
    }
    ...
    override fun exists_pos(tc: ThreadContext, key: Long): Long {
        ...
        if (key >= 0 && key < this.elems) {
            return if (Ops.isnull(this.slots!![start + key.toInt()]) == 0L) 1 else 0
        }
        return 0
    }
    ...
    private fun set_size_internal(tc: ThreadContext, n: Long) {
        ...
        if (ssize < 8192) {
            ssize *= 2
            if (n > ssize) ssize = n
            if (ssize < INIT_SIZE) ssize = INIT_SIZE.toLong()
        }
        else {
        ...
        this.slots = if (slots == null) arrayOfNulls(ssize.toInt())
    ...
    override fun splice(tc: ThreadContext, from: SixModelObject, offset: Long, count: Long) {
        ...
        var start: Long
        val tail: Long
        var slots: Array<SixModelObject?>?
        ...
        var tailCalc = elems0 - offset - count
        if (tailCalc < 0)
            tailCalc = 0

        else if (tailCalc > 0 && count > elems1) {
            /* We're shrinking the array, so first move the tail left */
            slots = this.slots
            start = this.start.toLong()
            memmove(slots!!, start + offset + elems1, start + offset + count, tailCalc)
        }
        tail = tailCalc
    ...
    override fun clone(tc: ThreadContext): SixModelObject {
        try {
            val clone = this.clone() as VMArrayInstance
            clone.sc = null
            if (this.slots != null)
