...
class CUnion : REPR() {
    ...
    override fun compose(tc: ThreadContext, st: STable, repr_info: SixModelObject) {
        ...
        val repr_data = CUnionREPRData()

        val mroLength = attr_info.elems(tc)
        val attrInfos = ArrayList<CUnionREPRData.AttrInfo>()
        for (i in mroLength - 1 downTo 0) {
            ...
            if (parents <= 1) {
                ...
                for (j in 0 until numAttrs) {
                    ...
                    val info = CUnionREPRData.AttrInfo()
    ...
    override fun allocate(tc: ThreadContext, st: STable): SixModelObject {
         ...
         * composed. */
        val obj = CUnionInstance()
        val repr_data = st.REPRData as CUnionREPRData
        obj.st = st
        try {
            @Suppress("DEPRECATION")
            obj.storage = repr_data.structureClass!!.newInstance() as Union
        }
    ...
    @Suppress("DEPRECATION")
    private fun generateStructClass(tc: ThreadContext, st: STable, fields: List<CUnionREPRData.AttrInfo>) {
        val reprData = st.REPRData as CUnionREPRData
        val cw = ClassWriter(ClassWriter.COMPUTE_MAXS or ClassWriter.COMPUTE_FRAMES)
        val className = "__CUnion__" + typeId++

        val attributes = fields.size

        // public $className extends com.sun.jna.Union implements com.sun.jna.Structure.ByReference { ... }
        cw.visit(BytecodeVersion.EMITTED, Opcodes.ACC_PUBLIC or Opcodes.ACC_SUPER, className, null, "com/sun/jna/Union", null)
    ...
    private fun typeDescriptor(tc: ThreadContext, info: CUnionREPRData.AttrInfo): String? {
