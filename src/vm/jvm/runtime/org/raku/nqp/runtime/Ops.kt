...
object Ops {
    ...
    fun checkcrlf(tgt: String?, pos: Long, eos: Long): Long {
        // NFG: a CR LF pair is a single synthetic grapheme, so there is no
        // separate LF at pos+1 to skip -- one grapheme, one position. This was
        // "return pos+1 when substr(pos,2) == \r\n"; in the grapheme model that
        // fix-up is a no-op (the grapheme's own advance covers the pair).
        return pos
    }
    ...
    private fun runNFA(tc: ThreadContext, nfa: NFAInstance, target: String, pos: Long): IntArray {
         ...
         * Cached per source -- runNFA fires once per parse position, so building
         * this array per call would be O(n^2) over a file (it stalled the
         * bootstrap on QAST.nqp). */
        val bases = NFG.baseCodepoints(target)
        val eos = bases.size
        var gen = 1
