...
QAST::OperationsJAST.map_classlib_core_op('note', $TYPE_OPS, 'note', [$RT_STR], $RT_STR, :tc);
...
class QAST::CompilerJAST {
    ...
    multi method as_jast(QAST::Block $node, :$want) {
        ...
        if !$*CODEREFS.know_cuid($node.cuid) {
            ...
            # Hand the method body whole to the code engine: an encoded
            # block's body is one codeRunIdx call (parameter binding
            # included), and there is no bytecode body to fall back to.
            # The program is a byte-framed entry of the unit record, named
            # by the block table's program index.
            my $body;
    ...
    multi method as_jast(QAST::Regex $node, :$want) {
        # Every regex rule is handed to the engine whole. The per-node bytecode
        # codegen was removed once every nqp and Rakudo grammar rule encoded to
        # the engine (a full NQP_RX_STRICT build proved it), so a rule the
        # descriptor cannot represent is a hard error rather than a silent
        # fallback -- RxDescriptor.bail and the descriptor size cap die naming
        # the rule and the reason.
        my $desc := self.rx_descriptor($node);
        nqp::die('QAST::Regex has no engine descriptor and the bytecode '
            ~ 'regex fallback has been removed') if nqp::isnull($desc);
        self.engine_jast($node, $desc)
    }

    # The descriptor for this rule. The engine is the only regex code path now,
    ...
    method rx_descriptor($node) {
        ...
        # bigger cannot be encoded and, with no bytecode fallback, cannot be
        # compiled at all -- fail naming it rather than emit a class that will
        # not load.
        my str $encoded := $desc.encoded;
        if nqp::chars($encoded) > 20000 {
            nqp::die("regex engine cannot encode rule '"
                ~ ($name eq '' ?? '<anon>' !! $name) ~ "': descriptor is "
                ~ nqp::chars($encoded) ~ " chars, over the 20000 class-file "
                ~ "constant limit");
        }
